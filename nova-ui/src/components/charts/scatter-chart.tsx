...
export function ScatterChart({
  ...
  const { xMin, xMax, yMin, yMax, sizeMin, sizeMax } = useMemo(() => {
    ...
    return {
      ...
      sizeMin: isFinite(sizeMin) ? sizeMin : 0,
      sizeMax: isFinite(sizeMax) ? sizeMax : 1,
    };
  }, [data, series]);
  ...
  // Precompute all points for nearest-point lookup
    ...
    <div ref={ref} className={cn("w-full", className)}>
        ...
        >
          ...
          {xTicks.map((t) => (
            <g key={`gx-${t}`}>
              <line
                ...
                y2={PAD.top + innerH}
                stroke="var(--nova-border)"
                strokeDasharray={t === xTicks[0] ? undefined : "3 3"}
              />
              <text
                x={sx(t)}
                y={PAD.top + innerH + 14}
          ...
          {series.map((s, si) => {
            ...
            return (
              <g key={`${s.key}-${s.yKey}`}>
                {data.map((d, di) => {
                  const cx = sx(Number(d[s.key]) || 0);
                  const cy = sy(Number(d[s.yKey]) || 0);
                  const r = s.sizeKey
                    ? sizeScale(Number(d[s.sizeKey]) || 0)
                    : FIXED_RADIUS;
      ...
      {hoverPoint && (
        <ChartTooltip
          ...
          rows={[
            {
              color: seriesColor(hoverPoint.si, hoverPoint.s.color),
              label: "x",
              value: formatX(Number(hoverPoint.d[hoverPoint.s.key]) || 0),
            },
            {
              color: seriesColor(hoverPoint.si, hoverPoint.s.color),
              label: "y",
              value: formatY(Number(hoverPoint.d[hoverPoint.s.yKey]) || 0),
            },
            ...(hoverPoint.s.sizeKey
              ? [
                  {
                    color: seriesColor(hoverPoint.si, hoverPoint.s.color),
                    label: hoverPoint.s.sizeKey,
                    value: defaultFormat(Number(hoverPoint.d[hoverPoint.s.sizeKey]) || 0),
                  },
                ]
              : []),
          ]}
